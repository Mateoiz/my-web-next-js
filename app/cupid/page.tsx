...
export default function CupidPage() {
  ...
  const handleAcceptRequest = async (request: MatchRequest) => {
    ...
    try {
        ...
        setTimeout(() => { 
            ...
            setTimeout(() => initializeChat(request.id, false, currentUser.id, matchData), 2000); 
        }, 1500);
  ...
  // --- UPDATED FIND MATCH: SENDS REQUESTS INSTEAD OF CONNECTING ---
  const findMatch = async (user: UserProfile, isReroll = false, excludeId?: string) => {
    ...
    if (new Date() < OPEN_DATE) {
        alert("Cupid's arrows are being sharpened! Matching opens on Feb 12.");
        return;
    }

    setState('SCANNING'); 

    // Note: We do NOT loadExistingMatch here anymore because "Find Match" now means "Find NEW people"
    ...
    try {
      ...
      let potentialMatches = snapshot.docs
        ...
        .filter(u => {
             ...
             return u.id !== user.id 
                 && (!user.currentMatchId || u.id !== user.currentMatchId) 
                 && (!excludeId || u.id !== excludeId) 
                 && (!isNaN(cAge) && cAge >= minAge && cAge <= maxAge)
                 && requests.length < 3 // Don't spam full inboxes
                 && !alreadyRequested;
        });
      ...
      if (potentialMatches.length > 0) {
          ...
          // Send Request
          await updateDoc(doc(db, "cupid_users", target.id!), {
              incomingRequests: arrayUnion(requestData)
          });
  ...
  // ... (handleConnect, handleLogin, handleSignup, handleImageSelect, removeImage, handleProfileSubmit, initializeChat, sendMessage, handleLogout same as before) ...
  const handleConnect = async () => { /* ... Unused in request flow but kept for safety ... */ };
  ...
  const renderTermsModal = () => (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-0 md:p-4 bg-black/90 backdrop-blur-md">
      <motion.div initial={{ y: "100%", opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-zinc-50 dark:bg-zinc-900 border-0 md:border border-zinc-200 dark:border-zinc-800 w-full h-full md:h-auto md:max-h-[85vh] md:max-w-lg md:rounded-3xl relative shadow-2xl flex flex-col overflow-hidden transition-colors">
        <div className="p-5 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center bg-zinc-100 dark:bg-zinc-900 z-10 shadow-sm transition-colors">
            <div><h3 className="text-xl font-black text-zinc-900 dark:text-white flex items-center gap-2 transition-colors"><FaFileContract className="text-rose-500" /> TERMS & RULES</h3><p className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold mt-0.5">Please read carefully</p></div>
  ...
  const renderForgotModal = () => (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="p-8 rounded-3xl max-w-sm w-full relative shadow-2xl transition-colors duration-300 bg-black border border-white/20 dark:bg-white dark:border-black/10">
        <button onClick={() => { setShowForgotModal(false); setResetStatus('idle'); }} className="absolute top-6 right-6 transition-colors text-white/50 hover:text-white dark:text-black/50 dark:hover:text-black"><FaTimes /></button>
        <h3 className="text-2xl font-black mb-2 text-white dark:text-black">RESET PASSWORD</h3>
        <p className="text-xs mb-6 text-white/60 dark:text-black/60">Enter your student email to receive a reset link.</p>
        {resetStatus === 'sent' ? (
            <div className="p-4 rounded-xl text-center bg-green-900/20 border border-green-500/50 dark:bg-green-100 dark:border-green-300">
                <FaCheckCircle className="text-3xl mx-auto mb-2 text-green-500 dark:text-green-600" />
                <p className="font-bold text-sm text-green-400 dark:text-green-700">Email Sent!</p>
                <p className="text-xs mt-1 text-green-400/60 dark:text-green-600/70">Check your inbox (and spam folder).</p>
                <button onClick={() => setShowForgotModal(false)} className="mt-4 text-xs underline text-white dark:text-black">Close</button>
            </div>
        ) : (
            <>
                <div className="relative mb-6 group">
                    <FaEnvelope className="absolute top-4 left-4 transition-colors text-white/50 group-focus-within:text-rose-500 dark:text-black/50 dark:group-focus-within:text-rose-600" />
                    <input type="email" placeholder="dlsau email..." value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} className="w-full rounded-xl p-4 pl-12 outline-none text-sm font-bold transition-all bg-zinc-900 border border-zinc-800 text-white placeholder:text-zinc-500 focus:border-rose-500 dark:bg-zinc-100 dark:border-zinc-300 dark:text-black dark:placeholder:text-zinc-400 dark:focus:border-rose-600" />
                </div>
                <button onClick={handleForgotPassword} disabled={resetStatus === 'sending'} className="w-full py-4 rounded-2xl font-black text-sm transition-all duration-300 shadow-lg border-0 bg-white text-black hover:bg-zinc-200 dark:bg-black dark:text-white dark:hover:bg-zinc-800">
                    {resetStatus === 'sending' ? <FaSpinner className="animate-spin mx-auto" /> : 'SEND LINK'}
                </button>
                {resetStatus === 'error' && <p className="text-xs font-bold mt-4 text-center text-red-500 dark:text-red-600">Failed to send. Check email.</p>}
            </>
        )}
      </motion.div>
    </div>
