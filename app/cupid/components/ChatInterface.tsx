...
export const ChatInterface = ({ match, currentUser, messages, newMessage, setNewMessage, onSendMessage, onStop, onNext, messagesEndRef, isPartnerDisconnected }: ChatInterfaceProps) => {
  ...
  // --- Confirmation State ---
  const [confirmModal, setConfirmModal] = useState<{ isOpen: boolean; action: 'stop' | 'next' | null }>({
    isOpen: false,
    action: null
  });
  ...
  const handleStopClick = () => {
    if (isPartnerDisconnected || !match) {
        onStop();
    } else {
        setConfirmModal({ isOpen: true, action: 'stop' });
    }
  };
  ...
  const handleNextClick = () => {
    if (isPartnerDisconnected || !match) {
        ...
        setConfirmModal({ isOpen: true, action: 'next' });
    }
  };

  const executeConfirmAction = () => {
    if (confirmModal.action === 'stop') onStop();
    if (confirmModal.action === 'next') onNext();
  ...
  return (
    ...
    <motion.div 
        ...
        className="fixed inset-0 z-[100] pt-24 md:pt-0 bg-zinc-50 dark:bg-black flex flex-col md:relative md:h-[80vh] md:w-full md:max-w-4xl md:mx-auto md:rounded-3xl md:overflow-hidden md:border md:border-zinc-200 md:dark:border-zinc-800 md:shadow-2xl"
    >
      ...
      <div className="p-4 bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center z-10 shadow-sm">
        ...
        <div className="flex gap-2">
            ...
            <button onClick={handleStopClick} className="px-4 py-2 rounded-xl bg-zinc-100 dark:bg-zinc-800 text-zinc-500 hover:text-red-500 font-bold text-xs uppercase tracking-wide transition-colors">Stop</button>
            <button onClick={handleNextClick} className="px-6 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-black text-xs uppercase tracking-wide shadow-lg shadow-rose-500/20 flex items-center gap-2">
                Next <FaStepForward />
            </button>
        </div>
      </div>
    ...
    <AnimatePresence>
        {confirmModal.isOpen && (
            <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md" onClick={() => setConfirmModal({isOpen: false, action: null})}>
                 <motion.div 
                    initial={{ scale: 0.9, opacity: 0 }} 
                    animate={{ scale: 1, opacity: 1 }} 
                    exit={{ scale: 0.9, opacity: 0 }}
                    onClick={(e) => e.stopPropagation()}
                    className="bg-white dark:bg-zinc-900 p-6 rounded-2xl shadow-2xl max-w-xs w-full text-center border border-zinc-200 dark:border-zinc-800"
                >
                    <div className="w-16 h-16 bg-yellow-100 dark:bg-yellow-900/30 rounded-full flex items-center justify-center mx-auto mb-4">
                        <FaExclamationTriangle className="text-2xl text-yellow-600 dark:text-yellow-500" />
                    </div>
                    <h3 className="text-xl font-black text-zinc-900 dark:text-white mb-2">End Chat?</h3>
                    <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6 font-medium leading-relaxed">
                        Are you sure you want to leave? You won't be able to message <span className="text-rose-500 font-bold">{match?.name}</span> again.
                    </p>
                    <div className="flex flex-col gap-2">
                        ...
                        <button 
                            ...
                            className="w-full py-3 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 font-bold rounded-xl hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors"
                        >
                            Cancel
                        </button>
                    </div>
    ...
    <AnimatePresence>
        {showProfile && match && (
            <div className="fixed inset-0 z-[110] pt-24 md:pt-0 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setShowProfile(false)}>
                <motion.div 
                    initial={{ scale: 0.9, opacity: 0 }} 
                    animate={{ scale: 1, opacity: 1 }} 
                    exit={{ scale: 0.9, opacity: 0 }}
                ...
                >
                    ...
                    <div className="px-6 pb-6 relative">
                        ...
                        {/* Centered Avatar */}
                        <div className="w-24 h-24 rounded-full border-4 border-white dark:border-zinc-900 overflow-hidden absolute -top-12 left-1/2 -translate-x-1/2 bg-zinc-200 shadow-md">
                             {match.imgs?.[0] ? <img src={match.imgs[0]} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-rose-500" />}
                        </div>
