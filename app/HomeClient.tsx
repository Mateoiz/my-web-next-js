...
export function HomeClient({ latestNews }: { latestNews: BlogPost[] }){
  ...
  return (
    <main className="min-h-screen relative selection:bg-green-500/30 bg-white dark:bg-black overflow-hidden font-sans">
      ...
      {/* ================= 2. LATEST NEWS (UPDATED TO USE LIVE DATA) ================= */}
      <motion.section 
        variants={sectionVariants}
        initial="hidden"
        whileInView="visible"
            ...
            {latestNews.length > 0 ? (
                ...
                <Link href={`/Blogs/${blog.slug}`} key={blog.id} className="group block">
                    ...
                    {blog.coverImage ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center bg-zinc-800 text-zinc-500 text-xs uppercase font-mono">No Image</div>
                    )}
                    </div>
                    <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-2 group-hover:text-green-600 dark:group-hover:text-green-500 transition-colors line-clamp-2">
                        {blog.title}
                    </h3>
                ...
                <div className="col-span-3 text-center py-10 border-2 border-dashed border-zinc-800 rounded-xl">
                    <p className="text-zinc-500 italic">No news updates available at the moment.</p>
                </div>
            )}
          </div>
        </div>
                         ...
                         <div className="flex items-center justify-between">
                            ...
                            <div className="flex gap-2">
                                {officer.socials.facebook && (
                                  <a href={officer.socials.facebook} target="_blank" rel="noopener noreferrer" className="p-2 rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-500 hover:text-white hover:bg-blue-600 transition-colors">
                                    <FaFacebook />
                                  </a>
                                )}
                                {officer.socials.email && (
                                  <a href={`mailto:${officer.socials.email}`} className="p-2 rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-500 hover:text-white hover:bg-green-600 transition-colors">
                                    <FaEnvelope />
                                  </a>
                                )}
                            </div>
