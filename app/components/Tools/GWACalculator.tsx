...
export default function GWACalculator() {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md border border-zinc-200 dark:border-green-500/30 rounded-2xl shadow-xl relative overflow-hidden p-5 md:p-8">
      ...
      <div className="pt-2 space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
        ...
        <div className="flex items-center gap-2 text-[10px] md:text-xs font-bold text-zinc-400 uppercase tracking-widest px-1">
          ...
          <span className="flex-1 text-center">Raw Score</span>
          <span className="w-16 md:w-20 text-center shrink-0">Units</span>
      ...
      <button onClick={addSubject} className="mt-4 w-full py-3 md:py-2 border-2 border-dashed border-zinc-300 dark:border-zinc-700 text-zinc-500 rounded-xl hover:border-green-500 hover:text-green-500 transition-colors text-xs font-bold uppercase tracking-widest flex items-center justify-center gap-2">
        <FaPlus /> Add Subject
      </button>
      
      <div className="flex gap-3 md:gap-4 mt-6 md:mt-8">
        <button onClick={calculateGWA} className="flex-1 bg-green-600 hover:bg-green-500 text-white font-bold py-3 md:py-4 rounded-xl shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2 text-sm uppercase tracking-wider">
      ...
      <AnimatePresence>
        ...
        {result.gwa !== null && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-6 pt-6 border-t border-zinc-200 dark:border-zinc-700 text-center">
            ...
            <div className={`text-4xl md:text-5xl font-black my-2 tracking-tighter ${result.gwa >= 3.0 ? "text-green-500" : "text-zinc-700 dark:text-white"}`}>{result.gwa.toFixed(4)}</div>
            {result.title ? <div className="inline-flex items-center gap-2 px-4 py-2 bg-yellow-500/10 border border-yellow-500/30 rounded-full text-yellow-600 dark:text-yellow-400 text-xs md:text-sm font-bold uppercase tracking-wide animate-pulse"><FaAward /> {result.title}</div> : <p className="text-zinc-400 text-xs mt-2">Keep pushing!</p>}
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
