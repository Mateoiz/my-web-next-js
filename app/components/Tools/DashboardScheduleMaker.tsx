  ...
  if (view === 'editor') {
    return (
      <div className="max-w-5xl mx-auto space-y-8 pb-20 animate-in fade-in slide-in-from-bottom-4">
        ...
        <div className="bg-white/40 dark:bg-zinc-900/40 backdrop-blur-xl border border-zinc-200 dark:border-zinc-800 rounded-[2rem] p-8 shadow-xl flex flex-col md:flex-row justify-between gap-6 relative overflow-hidden group">
          ...
          <div className="flex-1 space-y-4 relative z-10">
            <input 
              ...
              className="w-full text-3xl md:text-4xl font-black bg-transparent border-none outline-none placeholder:text-zinc-400 dark:placeholder:text-zinc-600 text-zinc-900 dark:text-white tracking-tight"
            />
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-zinc-100 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl text-sm font-bold uppercase tracking-widest text-[#06402B]">
              {classes.length} Classes Added
            </div>
          </div>

          <div className="shrink-0 relative z-10 flex flex-col justify-end">
            <button onClick={() => setView('canvas')} className="px-8 py-4 bg-[#06402B] text-white rounded-2xl font-black uppercase tracking-widest flex items-center justify-center gap-3 hover:scale-[1.02] active:scale-95 transition-all shadow-[0_0_20px_rgba(6,64,43,0.3)]">
            ...
            {classes.map((cls, index) => (
              ...
              >
                ...
                <div className="flex items-center gap-3 lg:w-48">
                  <button onClick={() => cycleColor(cls.id, cls.color)} className={`w-8 h-8 rounded-full ${cls.color} flex items-center justify-center text-white/50 hover:text-white transition-colors shadow-inner shrink-0`} title="Click to change color">
                    <FaPalette size={12} />
                  </button>
                  <input 
                    type="text" placeholder="Code (CS101)" value={cls.code} onChange={(e) => updateClass(cls.id, 'code', e.target.value)}
                    className="w-full bg-zinc-100 dark:bg-zinc-950/50 border border-zinc-200 dark:border-zinc-800 outline-none font-black text-zinc-900 dark:text-zinc-100 p-3 rounded-xl focus:border-[#06402B] transition-colors uppercase"
                  />
                </div>
                ...
                <div className="flex items-center gap-1 bg-zinc-100 dark:bg-zinc-950/50 p-1.5 rounded-xl border border-zinc-200 dark:border-zinc-800 shrink-0">
                  {DAYS_OF_WEEK.map(day => (
                    <button 
                      key={day} onClick={() => toggleDay(cls.id, day)}
                      className={`w-8 h-8 rounded-lg text-xs font-black transition-all ${cls.days.includes(day) ? `${cls.color} text-white shadow-md` : 'text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-800'}`}
                    >
                      {day}
                    </button>
                  ))}
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <input 
                    ...
                    className="w-28 bg-zinc-100 dark:bg-zinc-950/50 border border-zinc-200 dark:border-zinc-800 outline-none font-bold text-zinc-700 dark:text-zinc-300 p-2.5 rounded-xl focus:border-[#06402B] text-center text-sm"
                  />
                ...
                <button onClick={() => removeClass(cls.id)} className="text-zinc-300 dark:text-zinc-700 hover:text-red-500 transition-colors p-3 lg:p-2 opacity-100 lg:opacity-0 group-hover:opacity-100 shrink-0 flex justify-center items-center">
                  <FaTrashAlt />
                </button>
              </motion.div>
            ))}
      ...
      <div className="absolute inset-0 z-50 bg-zinc-50 dark:bg-zinc-950 flex flex-col">
        ...
        <div className="h-20 border-b border-zinc-200 dark:border-zinc-800 px-8 flex items-center justify-between shrink-0 bg-white/50 dark:bg-black/50 backdrop-blur-xl">
          <div className="flex items-center gap-4">
            <button onClick={() => setView('editor')} className="w-10 h-10 rounded-full bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all text-zinc-500">
              <FaTimes />
            </button>
            <div>
              <h3 className="font-black text-lg uppercase tracking-tight">{termName || "My Schedule"}</h3>
              <p className="text-[10px] font-mono font-bold text-[#06402B] uppercase tracking-widest">Academic Timetable</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-auto p-4 md:p-8 custom-scrollbar">
          <div className="min-w-[800px] w-full max-w-6xl mx-auto bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[2rem] shadow-2xl p-6 relative">
            ...
            <div className="grid grid-cols-7 gap-4 mb-4">
              ...
              {DAYS_OF_WEEK.map(day => {
                ...
                return (
                  <div key={day} className="col-span-1 text-center py-3 bg-zinc-100 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl">
                    <p className="font-black text-zinc-900 dark:text-white uppercase tracking-wider text-sm">{fullDay}</p>
                  </div>
                );
              })}
            </div>

            <div className="grid grid-cols-7 gap-4 relative h-[800px]">
              
              <div className="col-span-1 flex flex-col justify-between border-r border-dashed border-zinc-200 dark:border-zinc-800 pr-4">
                {Array.from({ length: END_HOUR - START_HOUR + 1 }).map((_, i) => {
                  const hour = START_HOUR + i;
                  const ampm = hour >= 12 ? 'PM' : 'AM';
                  const displayHr = hour > 12 ? hour - 12 : hour;
                  return (
                    <div key={i} className="text-right text-[10px] font-mono font-bold text-zinc-400 uppercase relative -top-2">
                      {displayHr}:00 {ampm}
                    </div>
                  );
                })}
                  ...
                  {classes.filter(c => c.days.includes(day)).map(cls => {
                    ...
                    return (
                      <motion.div 
                        ...
                        className={`absolute left-0 right-0 mx-1 rounded-xl p-3 shadow-lg flex flex-col overflow-hidden text-white ${cls.color}`}
                        style={{ top: pos.top, height: pos.height }}
                      >
                        <div className="absolute inset-0 bg-black/10 mix-blend-overlay pointer-events-none" />
                        <h4 className="font-black text-sm md:text-base leading-tight relative z-10">{cls.code}</h4>
                        <p className="text-[10px] font-bold text-white/80 uppercase tracking-widest mt-0.5 truncate relative z-10">{cls.name}</p>
                        <p className="text-[10px] font-mono font-bold mt-auto opacity-80 relative z-10">
