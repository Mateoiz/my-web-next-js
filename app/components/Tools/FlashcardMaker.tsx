...
{/* ── Editor breadcrumb + actions bar ── */}
  ...
  if (view === 'study') {
    ...
    return (
      <div className="fixed inset-0 z-50 bg-zinc-50 dark:bg-zinc-950 flex flex-col md:absolute">
  ...
  <div className="border-t border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl px-4 md:px-8 py-4 pb-[calc(1rem+env(safe-area-inset-bottom))] md:pb-4 flex items-center justify-center gap-3 shrink-0">
    <button onClick={() => handleNext(false)} disabled={studyMode === 'flip' && !isFlipped}
      className="flex-1 sm:flex-none sm:w-44 py-3.5 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 rounded-2xl font-black uppercase tracking-widest flex items-center justify-center gap-2 disabled:opacity-30 hover:bg-orange-500 hover:text-white dark:hover:bg-orange-500 transition-all text-xs touch-manipulation"
    >
      <FaRedo size={11} /> Again
    </button>
    <button onClick={() => handleNext(true)} disabled={studyMode === 'flip' && !isFlipped}
      className="flex-1 sm:flex-none sm:w-44 py-3.5 bg-[#06402B] dark:bg-emerald-600 text-white rounded-2xl font-black uppercase tracking-widest flex items-center justify-center gap-2 disabled:opacity-30 hover:bg-[#042d1f] dark:hover:bg-emerald-500 active:scale-95 transition-all shadow-md text-xs touch-manipulation"
    >
