...
import { FaHeart, FaKeyboard, FaTrophy, FaUser } from "react-icons/fa";
...
export default function ValentineTyper() {
  ...
  // Database Logic
  useEffect(() => {
    if (status === 'finished' && !isSaved && wpm > 0) {
      saveScore(wpm, accuracy);
    }
    if (status === 'idle') setIsSaved(false);
  }, [status, wpm, accuracy, isSaved]);

  useEffect(() => { fetchLeaderboard(); }, []);

  const saveScore = async (score: number, acc: number) => {
  ...
  return (
    // CHANGED: Changed justify-center to justify-start and added pt-36 for top padding
    <section className="relative w-full min-h-screen flex flex-col items-center justify-start pt-36 pb-12 px-4 md:px-8 overflow-hidden">
      ...
      <div className="grid lg:grid-cols-12 gap-8 relative z-10 w-full max-w-[1400px]">
        ...
        >
             ...
             <div className="font-mono text-2xl md:text-3xl leading-relaxed break-words select-none">
                {text.split('').map((char, index) => {
                    ...
                    if (index < input.length) {
                        if (inputChar === char) {
                            className = "text-zinc-800 dark:text-rose-400 shadow-[0_0_15px_rgba(244,63,94,0.4)]"; 
                        } else {
                            className = "text-red-500 bg-red-500/10 underline decoration-4 decoration-red-500"; 
                        }
                    }
          ...
          >
             {status === 'finished' ? (isSaved ? "Data Uploaded // Restart" : "Processing...") : "Restart System"}
          </button>
        </motion.div>
              ...
              <div className="space-y-4 overflow-y-auto pr-2 custom-scrollbar flex-1">
                 {leaderboard.length === 0 ? (
                    ...
                    leaderboard.map((entry, i) => (
                       <div key={i} className="group relative flex items-center justify-between p-4 rounded-xl bg-zinc-100 dark:bg-black/40 border border-transparent hover:border-rose-500/50 hover:bg-rose-500/5 transition-all">
