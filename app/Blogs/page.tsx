...
export default function BlogPage() {
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-zinc-950 text-black dark:text-zinc-100 font-sans pt-32 pb-20 transition-colors duration-300">
      
      <div className="max-w-7xl mx-auto px-6">
        
        {/* --- HEADER --- */}
        <div className="flex flex-col md:flex-row items-end justify-between gap-8 mb-16 border-b-2 border-black dark:border-zinc-800 pb-8 relative">
          
          {/* WRITER ACCESS BUTTON (Top Right of Header) */}
          <Link 
            href="/signup" 
            className="absolute top-0 right-0 hidden md:flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-zinc-400 hover:text-green-600 dark:hover:text-green-400 transition-colors"
          >
          ...
          <div className="w-full md:w-auto relative group">
            ...
            <input 
                ...
                className="w-full md:w-80 pl-10 pr-4 py-4 bg-zinc-50 dark:bg-zinc-900 border-2 border-black dark:border-zinc-700 font-bold text-sm outline-none focus:bg-white dark:focus:bg-black transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-none focus:translate-y-[2px] focus:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
            />
          </div>
        </div>
        ...
        {loading && (
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[1, 2, 3].map((i) => (
                 <div key={i} className="h-96 bg-zinc-100 dark:bg-zinc-900 animate-pulse border-2 border-zinc-200 dark:border-zinc-800 rounded-none" />
              ))}
           </div>
        )}
        ...
        {!loading && filteredPosts.length === 0 && (
            <div className="py-20 text-center border-2 border-dashed border-zinc-300 dark:border-zinc-800 rounded-xl">
                ...
                <p className="text-zinc-400 font-bold uppercase tracking-widest">No transmissions found matching query.</p>
            </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {!loading && filteredPosts.map((post) => (
            <Link href={`/blog/${post.slug}`} key={post.id} className="group block h-full">
              ...
              >
                {/* Image */}
                <div className="relative h-56 w-full border-b-2 border-black dark:border-zinc-800 overflow-hidden bg-zinc-100 dark:bg-zinc-800">
                  {post.coverImage ? (
                    <Image 
                        src={post.coverImage} 
                        alt={post.title}
                ...
                <div className="flex flex-col flex-grow p-6">
                  <div className="flex items-center gap-3 text-xs font-bold font-mono text-zinc-500 dark:text-zinc-400 mb-3 uppercase tracking-wide">
                    ...
                    <span className="flex items-center gap-1 text-black dark:text-zinc-200"><FaUser /> {post.author}</span>
                  </div>

                  <h3 className="text-xl font-black text-black dark:text-white leading-tight mb-3 group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors">
                    {post.title}
                  </h3>
